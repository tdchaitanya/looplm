...
class CommandHandler:
    ...
    def _handle_model(self) -> bool:
        ...
        for provider, config in providers.items():
            provider_name = self.config_manager.get_provider_display_name(provider, config)
        ...
        if provider_input:
            ...
            self.console.display_success(f"Switched to {provider_input} with model {model_input}")
        else:
            model_input = Prompt.ask("Enter model name")
    ...
    def _handle_system(self) -> bool:
        ...
        if choice == "1":
            ...
            save = Prompt.ask("Save this prompt? (y/n)", choices=["y", "n"], default="n")
            ...
            if self.prompts_manager.delete_prompt(name):
                ...
                self.console.display_error("Cannot delete prompt (not found or is default)")
    ...
    def start_session(self):
        ...
        while True:
            try:
                ...
                response = self.session_manager.active_session.send_message(
                    user_input,
                    stream=True,
                    show_tokens=False
                )
        ...
        if self.override_provider:
            try:
                ...
                try:
                    ...
                    if other_config and other_config.get("provider_name") == self.override_provider:
