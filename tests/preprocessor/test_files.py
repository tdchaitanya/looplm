...
class TestFilePreprocessor:
    ...
    def test_resolve_path_relative(self, preprocessor, sample_files):
        ...
        rel_path = os.path.relpath(
            sample_files["txt_file"], 
            start=preprocessor.base_path
        )
    ...
    def test_handle_url(self, preprocessor, mock_requests):
        ...
        mock_requests.assert_called_once_with(
            url,
            headers={"User-Agent": "Mozilla/5.0 (compatible; LoopLM/1.0)", 
    ...
    def test_handle_url_too_large(self, preprocessor, mock_requests):
        ...
        with patch("pathlib.Path.stat") as mock_stat:
            ...
            sizes = [0, 5*1024*1024, 9*1024*1024, 11*1024*1024]  # 0, 5MB, 9MB, 11MB
    ...
    def test_handle_unsupported_file_format(self, preprocessor, mock_markitdown, temp_dir):
        ...
        mock_instance.convert.side_effect = Exception("General error, not UnsupportedFormatException") 
        
        with pytest.raises(ValueError, match="Unsupported file format or conversion error"):
            preprocessor._handle_local_file(str(binary_file))

    def test_handle_unsupported_format_exception(self, preprocessor, mock_markitdown, temp_dir):
        ...
        mock_class, mock_instance = mock_markitdown
        mock_instance.convert.side_effect = UnsupportedFormatException("Unsupported format")
        
        with pytest.raises(FileProcessingError, match="File format '.xyz' is not supported"):
    ...
    def test_markitdown_unsupported_format(self, preprocessor, mock_markitdown, temp_dir):
        ...
        with pytest.raises(FileProcessingError, match="File format .* is not supported"):
            preprocessor._handle_local_file(str(unsupported_file))